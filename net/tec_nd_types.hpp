// Time-stamp: <Last changed 2026-02-20 16:22:32 by magnolia>
/*----------------------------------------------------------------------
------------------------------------------------------------------------
...
struct NdTypes {
    ...
    using ID = uint16_t; ///< Unique identifier type for serialized objects/messages.
    using Tag = uint16_t; ///< Tag type used to encode element kind and properties.
    using Size = uint32_t; ///< Size type for individual element payload (in bytes).
    using Count = uint16_t; ///< Count type for number of elements in sequences/containers.
    using Bool = uint8_t; ///< Boolean storage type (always 1 byte).
    using String = std::string; ///< String storage type.
    ...
    struct Meta {
        ...
        static constexpr Tag Scalar{(1 << 8)}; ///< Indicates a scalar value (single primitive).
        static constexpr Tag Float{(1 << 9)}; ///< Indicates a floating-point scalar.
        static constexpr Tag Signed{(1 << 10)}; ///< Indicates a signed integer scalar.
        static constexpr Tag Sequence{(1 << 11)}; ///< Indicates a sequence of scalar elements (e.g., byte array, string).
    };
     ...
     */
    struct Tags {
        static constexpr Tag Unknown{0}; ///< Unknown or uninitialized element.

        // Scalars
        static constexpr Tag I8{(1 | Meta::Scalar)}; ///< Signed 8-bit integer.
        static constexpr Tag I16{(2 | Meta::Scalar)}; ///< Signed/unsigned 16-bit integer.
        static constexpr Tag I32{(3 | Meta::Scalar)}; ///< Signed/unsigned 32-bit integer.
        static constexpr Tag I64{(4 | Meta::Scalar)}; ///< Signed/unsigned 64-bit integer.
        static constexpr Tag IBool{(5 | Meta::Scalar)}; ///< Boolean value.
        static constexpr Tag F32{(6 | Meta::Scalar | Meta::Float)}; ///< 32-bit IEEE-754 float.
        static constexpr Tag F64{(7 | Meta::Scalar | Meta::Float)}; ///< 64-bit IEEE-754 double.
        static constexpr Tag F128{(8 | Meta::Scalar | Meta::Float)}; ///< 128-bit floating point (platform-dependent).

        // Scalar sequences
        static constexpr Tag SByte{('B' | Meta::Scalar | Meta::Sequence)}; ///< Sequence of raw bytes (Blob).
        static constexpr Tag SChar{('A' | Meta::Scalar | Meta::Sequence)}; ///< Sequence of characters (std::string).

        // Containers
        static constexpr Tag Container{'C'}; // 67 ///< Flat container (vector, array, etc.).
        static constexpr Tag Map{'M'}; // 77 ///< Associative map/container.

        // Object
        static constexpr Tag Object{'O'}; // 79 ///< User-defined serializable object.
    };
...
#pragma pack(push, 1)
    ...
    struct Header {
        ...
        uint32_t magic; ///< Magic identifier (must match kMagic).
        uint32_t size; ///< Total size of the serialized payload **excluding** this header (bytes).
        uint16_t version; ///< Protocol version.
        uint16_t id; ///< User-defined root object identifier.
        int16_t  status; ///< Status code (application-specific).
        uint16_t compression_flags; ///< Compression type and level (low 4 bits: type, high 4 bits: level).
        uint32_t size_uncompressed; ///< Original uncompressed payload size (for verification).
     ...
     * | 6–7    | 2       | `count`  | Count (uint16_t)| Number of elements:<br>- 1 for scalars and sequences<br>- Actual count for containers (clamped to 65535)      |
     *
     * This layout is guaranteed to be contiguous and exactly the sizes shown on all platforms due to the packing directive.
     *
     */
    struct ElemHeader {
        Tag tag; ///< Type and property tag.
        Size size; ///< Payload size in bytes (excluding this header).
        Count count; ///< Number of elements (for containers/sequences; 1 for scalars).
